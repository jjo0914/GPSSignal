...
%   요약 : 이 함수는 GPS Legacy 네비게이션 비트 (LNAV) 또는 민간  네비게이션 비트(CNAV)를  생성합니다.
%   (그래도 LNAV가 주류래)

% cfg=는 함수 HelperGPSNavigationConfig을 통해 만들어진 값을 인자로 받은거라..
% 사용법은 cfg. 이긴하네
if strcmp(cfg.SignalType,'CNAV') || strcmp(cfg.SignalType,'L5') 
    bits = GPSCNAVDataEncode(cfg);  % -> CNAV 코드로
elseif strcmp(cfg.SignalType,'LNAV')
    bits = GPSLNAVDataEncode(cfg);   % -> LNAV코드로
else % strcmp(cfg.SignalType,'CNAV2')
    bits = GPSCNAV2DataEncode(cfg);  % -> CNAV2 코드로,, 아직 사용X
end
end
...
% 함수 matlabshared.internal.gnss.readSEMAlmanac 는 매트랩 toolbox 내장함수 readSEMAlmanc을 실행하는건데.. 궁금하면 코드 뜯어보든지..?
% 알마낙 파일은 SEM형식을 지켜야함!
[~, almWeekNum, almTimeOfApplicability, almStruct] = ...
    matlabshared.internal.gnss.readSEMAlmanac(cfg.AlmanacFileName); % 알마낙 파일명은 gpsAlmanac.txt

subframeLength = 300; % 서브프레임 Bits 300개
NumSubframes   = 5;  % LNAV 메시지는 총 서브프레임5개 
frameIndices = cfg.FrameIndices; % frameInidces=1~25 , 3~27, 5,29
numFrames = length(frameIndices); % 총생성할 프레임 갯수
navData = zeros(numFrames*NumSubframes*subframeLength, 1); % navData= 생성할 총 비트 수..

for iFrame = 1:numFrames
    cfg.PageID = mod(cfg.FrameIndices(iFrame)-1, 25)+1; % Subframe4부터 pageid를 사용하는데
                                                        % 첫1프레임은 pageid 1 , 2프레임부터 pageid 2                                                     
                                                        % 굳이 mod로 하는 이유는 프레임인다이스가 25를 넘어가면 다시 1부터 순환되게하려고한데
                                                        % PageID는 항상 25보다작아야한데..(GPS표준)..
    ...
    for iSubframe = 1:NumSubframes
        ...
        switch iSubframe
            case 1
                subframe = subframe1(cfg); % -> 여기서 선언한 함수네 112줄로 ㄱ
            case 2
                subframe = subframe2(cfg); % ->  195 줄로 ㄱ
            case 3
                subframe = subframe3(cfg); % -> 285 줄로 ㄱ
            case 4 % 서브프레임 4,5부터 알마낙 정보 포함
                subframe = subframe4(cfg,almStruct,almTimeOfApplicability); % ->376줄
            case 5 
                subframe = subframe5(cfg,almStruct,almTimeOfApplicability,almWeekNum); % -> 655줄로 ㄱ
        end
        cfg.HOWTOW = cfg.HOWTOW + 1;        % The TOW in HOW increments by 1 for every 6 sec.
                                            % HOWTOW 초기값은 비트가 보내지는 시간..
                                            % LNAV 서브프레임은 6초라서 한번 서브프레임 생성이 끝나면 6초증가
        startIDX = (iFrame-1)*(1500) + 1 + (iSubframe-1)*300; % 총비트내에서 단순 서브프레임 위치계산
        endIDX = (iFrame-1)*(1500) + iSubframe*300; 
        navData(startIDX:endIDX) = subframe; % 만들어진 서브프레임 비트를 navData배열에 옮김
    end
end
end
...
towBin = num2bits(howdata.HOWTOW, 17, 1); % HOWTOW(1주기준으로 얼마나지났는지..(6초간격))를 비트로 (17bit)
bit18Alert = howdata.AlertFlag;          % AlerFlag 위성 이상시 1 (18번bit)
bit19AS = howdata.AntiSpoofFlag;         % P코드 암호화여부 0:OFF (19 번째 bit)
subframeIDBin = num2bits(howdata.SubframeID, 3, 1); % subframe id 가 Navigationcofig 함수의 변수였노.,.?
                                                    % 이게 왜 LNAV메시지에들어가노?
data = [towBin; bit18Alert; bit19AS; subframeIDBin];
wordNumber = 2;
word = gpsLNavWordEnc(data,wordNumber);   % -> 오류검출용 패리티비트 만들기..
end
...
word1 = TLMWord(opts); % Having the de  fault values of TLM word for speed          
                       % -> 872번째줄로
                       %(동기화 역할,고정된 비트 생성)각 서브프레임의 첫번째  word를 생성함
word2 = HandoverWord(opts); % ->(85번째줄로) HOW word 생성

wordNumber = 3;       

weekNumber = mod(opts.WeekNumber,1024); % word 3부터 80년이후로 언제 시작하는지 알려주는 WEEKNUMber 
weekNumberBin = num2bits(weekNumber,10, 1); % 길이는 10비트

if strcmp(opts.CodesOnL2,'P-code') % L1밴드에는 C/A 코드가 항상들어가는데 
    codesOnL2Bin = [0;1];          % L2밴드에서는 P코드 사용여부를 알려줘야한데 (L2는원래 P코드 전용이긴했음
else % C/A-code
    codesOnL2Bin = [1;0];         % L2는 C/A코드를 안쓴다고하는데.. 다른 코드 일경우
end

uraVal = opts.URAID;  % URAID는 정확도오차 LEVLE을 나타냄
uraValBin = num2bits(uraVal, 4, 1);

svHealthBin = num2bits(opts.SVHealth, 6, 1); % Svhelath: 위성정상상태 =0정상

iodc = opts.IssueOfDataClock;  %  위성시계를 탑재하고 빵 쌋는데 상대성이론에의해 서로 인지하는 시간이 다르게 간다.. 그것을 보정하는 계수들의 버전기록
iodcBin = num2bits(iodc, 10, 1);

data = [weekNumberBin; codesOnL2Bin; uraValBin; svHealthBin; iodcBin(1:2)]; % word 3에서는 issuofdatclock을 앞2비트만 사용하고 나머지 8비트는 word8에 탑재
word3 = gpsLNavWordEnc(data,wordNumber);

wordNumber = 4;
data = [opts.L2PDataFlag; zeros(23, 1)]; % L2PDataFlag=L2밴드에서 P코드를 보낼 때 네비게이션  데이터를 보낼것인가?(1이면 0비트1500개나 의미없는 서브프레임들을보낸다)
...
T_GDBin = num2bits(opts.GroupDelayDifferential, 8, 2^-31); % GroupdelayDifrrentail 값은 보통 나노초수준인데 이걸 2^-31로 나눠줌 (GPS표준)
                                                           % 그리고 비트변환(GPS표준)
...
word1 = TLMWord(opts); % (869줄)Having the default values of TLM word for speed
                       %  (동기화 역할,고정된 비트 생성)각 서브프레임의 첫번째  word를 생성함
word2 = HandoverWord(opts); %같고..

wordNumber = 3;

iode = opts.IssueOfDataEphemeris; % 서브프레임2와 서브프레임3에서만 이값이 항상일치하면 ok 다르게 지정해도 무방?!
iodeBin = num2bits(iode,8,1);


crs = opts.HarmonicCorrectionTerms(3); % 위성궤도 계산 보정 항 6개..
...
meanMotionDiff = opts.MeanMotionDifference;    %위성궤도 평균각속도의 보정값
meanMotionDiffBin = num2bits(meanMotionDiff, 16, 2^(-43));

meanAnom = opts.MeanAnomaly;  % t_oe 시간에서의 위성궤도각도
...
eccen = opts.Eccentricity; % 위성궤도 이심률
scaleFactor = 2^(-33);
eccenNumBits = 32;
...
sqRootA = sqrt(opts.SemiMajorAxisLength); % 위성궤도 반장경길이 의 루트
scaleFactor = 2^(-19);
sqRootANumBits = 32;
...
toe = opts.ReferenceTimeOfEphemeris; % t_oe 타원궤도값들이 측정된시간
scaleFactor = 2^(4);
toeNumBits = 16;
toeBin = num2bits(toe, toeNumBits, scaleFactor);

AODO = opts.AgeOfDataOffset;   % subframe1에 시간보정 계수들 있잖아 그데이터가 얼마나 오래되었는지(최대17시간)
...
cis = opts.HarmonicCorrectionTerms(1);  % 위성궤도 계산 보정항들 6개 
...
longOrbitPlane = opts.LongitudeOfAscendingNode; % 지구 적도면과 위성궤도면 교차점을 나타내기위한 각도 오메가
                                                % 기준선은 그리니치 자오선
scaleFactor = 2^(-31);
...
refIncAngle = opts.Inclination;  %적도면과 위성궤도면 사이의 각도 0.3 ->
scaleFactor = 2^(-31);   % 소숫점 10자리까지 정수로변환가능
...
argPerig = opts.ArgumentOfPerigee; % 1=180도 -1=-180도
argPerigNumBits = 32;
scaleFactor = 2^(-31);
...
rorAscension = opts.RateOfRightAscension; % 오메가의 변화율 오메가는 살짝식 변한다
...
iode = opts.IssueOfDataEphemeris; % 궤도데이터 버전 보통은 t_oe 랑 256 mod 한다
iodeNumBits = 8;
iodeBin = num2bits(iode, iodeNumBits, 1);

idot = opts.InclinationRate;  % 궤도면과 적도면사이의 기울기변화율
                              % 단위 semicircle/s
idotNumBits = 14;
scaleFactor = 2^(-43);
...
dataid = [0; 1];  % dataid 01='알마낙 정보를 뿌리겟다'
            
subframe4SVID = [57; 25; 26; 27; 28; 57; 29; 30; 31; 32; 57; 62; 52; 53;...
    54; 57; 55; 56; 58; 59; 57; 60; 61; 62; 63]; % SVID는 위성을 식별하는 PRN번호 (SVID=PRN)
                                                 % 임의변경가능
                                                 % 인덱스가 pageID

switch opts.PageID %서브프레임4부터 PageID라는게 존재함 첫프레임은 1PageID / 마지막 25프레임은 25PageId를 전송
    ...
    case 13 % NMCT data
        ...
        nmctBin = NMCT2Bin(opts.NMCTERD); %NMCTERD는 각PAGEID에대한 오차예상값  
                                          %NMCT2Bin 함수 30pageid * 6비트 배열반환
    ...
    case 17 % Text message
        ...
        if length(fullText) >= numCharInMessage
            ...
            text = [fullText, repmat(' ',1, numCharInMessage-length(fullText))]; % 문자형 배열은 서로합침..
        end
        TempBits = int2bit(double(text),8); % 문자들을 하나씩 아스키값으로 바꾸고
                                            % 각 아스키값들을 8비트 행렬여러개로만듬
                                            % 8(비트)*22(문자) 비트값 모음
    ...
    case 18 % Ionospheric and UTC data
        ionosphere = opts.Ionosphere; %이온층 지연 시간보정계수 총8개가있음 
                                      % 계산식 GPT참고
        utc = opts.UTC;            %GPS시간과  UTC시간을 맞추기위한 윤초
                                   % 윤초가뭐냐? gpt참고/지구 시간은 계속 임의로 1초씩 넣어주고있다고함
                                   % 변수 ㅈㄴ 많으니 config코드참고
                                   % 단순 윤초를 빼는게아니라 시간보정계수들이있음..
        wordNum = 3;
        ...
        A1 = utc.UTCTimeCoefficients(2);  %윤초
    ...
    case 25 % A-S flags/ SV configurations for 32 SVs, plus SV health for SV 25 through 32
        ...
        for iSV = 1:32 % Generate SVConfig for each PRN
            ...
            eval([varName '=svconfigBin;']); % 동적변수 만드는거라는데 (비추천)
                                             % svconfigBing1, 2... 이렇게생성한데
        end
...
word1 = TLMWord(opts); % -> 870줄로 Having the default values of TLM word for speed
word2 = HandoverWord(opts);
...
caseName = 'Empty';
if ismember(opts.PageID, 1:24) % PageId 각 원소가 1:24에 있는가? [ 1 1 1 1 1 11  1 1 1 1 1 1]
    caseName = 'almanac';
elseif opts.PageID==25     % PageId가 25일경우 다른값
    caseName = 'SVData';
end
...
switch caseName
    ...
    case 'SVData'
        ...
        for iSV = 1:24 % Generate SVHealth for each PRN
            ...
            svhealthBin = num2bits(svhealth, 6, 1); %#ok<NASGU> 
            varName = ['svhealthBin' num2str(iSV)];
            eval([varName '=svhealthBin;']); % eval는 안에 ''문자열을 코드로 실행
        end
...
if nnz(almdataidx)==0
    ...
    %만약 해당 PRNid가 전부 0이라면 더미메시지를 넣는다
    % 더미 메시지 WORD3:010000001010101010101010+패리티
    %            WORD4: 101010101010101010101010+패리티
    svidBin = zeros(6, 1);
...
% 요약 : 이 함수는 LNAV 서브프레임의 동기화 비트 Word(TLM)를 생성하고, 옵션에 따라 기본값 또는 사용자 지정 데이터를 사용함.

preamble = num2bits(tlmdata.Preamble, 8, 1); % 10001011 =고정값 프래임블 139를 비트로
TLMMessage = num2bits(tlmdata.TLMMessage, 14, 1); % 000000000000 고정?
flags = [tlmdata.IntegrityStatusFlag; 0];    %  IntegrityStatusFlag : TLM word에서 위성의 정상상태표시(0정상)
data = [preamble; TLMMessage; flags];       
wordNumber = 1;                          % TLM Word는 항상 wordnumber=1 (서브프레임 내에서 1번째로 생성 된다는 의미)
word = gpsLNavWordEnc(data,wordNumber);  % 마지막 오류검출용 패리티 비트 계산법
end
...
numBits = 6;
scaleFactor = 0.3;
nmctBin = zeros(numeleInNMCT, numBits); % 30*6 배열
...
% 요약: WORDNumber가 2또는 10이다 22비트를입력으로 받고 30비트로만들어줌
%     WORDNumber가 1,3~9 이면 24비트를 입력으로 받고 30비트로 만들어줌
persistent d29Star d30Star %GPSLNAVWordEnc는 여러번 호출되니깐 d29star d30star값들을 초기화 하지않고 계속 기억하겠다..
               % d29star 이전 word의 29번재비트
               % d30star 이전 word의 30번째비트
               % 연쇄형 오류검출 비트로인해 신뢰성 상승
if wordNumber==1 || isempty(d29Star)
    d29Star=0;
    d30Star=0;
end
% Parity Polynomials for bits 25 to 30
% d25.. d30은 오류검출용 마지막 6비트를 만들때 어떤 비트를 참조해서 xor한건지 배열로저장
% gps표준
...
if wordNumber~=2 && wordNumber~=10 % Word number가 1 ,3~9 일때 (입력비트24비트)
    %Vectorize parity application to data
     dataParity = d30Star*ones(24, 1); % 이전 워드의 30번째비트를 모든 입력비트(24bit)와 더하겠다..

    %Generate Codeword based on the parity equations from the ICD -
    %Hamming(32,26)
    % 계산방법 : 데이터비트 (24비트) + 이전워드의 30번째비트 (24비트) + 이전워드29번째비트+d25인덱스끼리 xor(1비트) +..
    % XOR는 비트끼리더하고 2로나눈 나머지..
...
if wordNumber==2 || wordNumber==10 % 만약 2번째 WORD랑 10번째 WORD 일 경우는 좀다르다
    %Vectorize Parity application to data
    dataParity = d30Star*ones(22, 1); % 입력이 22비트~

    %Bits 23 and 24 have to be calculated by setting D29 and D30 to zero
    % word 2랑 word 10 에서는 29번째 30번째 비트가 계산결과가 0 이되게해야 함
    % 그걸 위한 공식인데.. XOR 역산도 그냥 mod하면된다,..
    ...
    codeWord = mod([dataParity+data(1:22); d30Star+bit23;...
        ...
        d30Star+sum(data(d28(d28<23)))+bit23; 0; 0],2); % 29 30번째 비트는 항상0
...
for idx = 1:numSequence
    ...
    if(strcmp(cfg.SignalType,'CNAV'))
     ...
     % For every 6 seconds, 17 MSBs of TOW gets incremented by 1. As each
    % message is 16 seconds long, TOW must be incremented by 1 for L5
    % signal
       cfg.HOWTOW = TOW + (idx-1);
    end
...
% Output:
%  y - Bits vector
                             % 2진수에서 음수는 양수의 보수형태로나타냄 둘이 더하면 0!(ex 5=000101 ,-5=111011)
y = int2bit(round(x./s),n);  % x가 10진수값 n이 출력할비트수, s는 스케일팩터라는데
                             % 여기서 스케일 팩터가 뭐냐? 
                             % 여러 물리량이 실수이기때문에.. 시간초처럼..
                             % 단위를 미리 정해서 비트수를 낮추기위함(GPS표준은 2^-31이다..)
