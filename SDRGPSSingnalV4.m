...
almFileName   = "current_sem.txt";  % 간단하다운 sem.txt (https://www.navcen.uscg.gov/gps-nanus-almanacs-opsadvisories-sof)
...
sampleRate        =  2*1e+06; %  fs> 4*C/A 칩레이트 1.023MHZ / 2*1.023e+06;
                               % 1023000   /
...
for istep = 1:numsteps
     ...
     if CodeBlockNumber == 0
         ...
         OnebitWaveform = wavegenobj(navdata(bitidx,:));%   40920샘플         % OnebitWaveform = imag(OnebitWaveform)+1i*real(OnebitWaveform); %ㅇ
...
% 사실 1이 2048값인거임
int16gps=round(gpswaveform(1:end).*32768); % !!수정 앞에 0제거(samprate*0.1) or 
%int16gps= max(min(int16gps, 2047), -2048);  % 12비트 범위로 이렇게하면 복소수라안댐
int16gps=int16(int16gps);    
% 우분투로저장
UbuntuFileSave;
